...
if __name__ == '__main__':
    ...
    offsets = [[20., 50.]]
    #offsets = []
    boost_SNRs=[dict(dayglow=75., nightglow=75.), dict(dayglow=1., nightglow=1.), ]
    boost_SNRs=[dict(dayglow=75., nightglow=75.), dict(dayglow=25., nightglow=25.), dict(dayglow=1., nightglow=1.), ]
    #airglow_considered=['nightglow',]
    #airglow_considered=['dayglow',]
    alt_scaling_airglow = 0. # (km) at which the scaling velocity-to-airglow was computed. Marouchka uses 0 as a reference
    alt_ref_balloon = 90. # (km) altitude at which we will use the TL later when computing the probabilities 
    #airglow_considered=[['nightglow', 'dayglow',]]
    save_max_dist = False
    all_airglow_considered = [['dayglow',], ['nightglow',],]

    for boost_SNR in boost_SNRs:
        for airglow_considered in all_airglow_considered:
            #for model_subsurface in ['Cold100', 'Hot10', 'Hot25', 'Hot40', ]:
            for model_subsurface in ['Cold100']:
                ...
                #file_precomputed_scaling = f'{main_dir}data/scaling_airglow/data_airglow_scaling_{model_subsurface}.csv'
                ...
                plt.figure(); 
                for key in f_alt_scaling_dayglow:
                    plt.plot(distances, f_alt_scaling_dayglow[key](distances), label=key); 
                plt.legend()
                plt.savefig('./figures/test_scaling.png')
                """
                ...
                if use_airglow_max_dist: ## Airglow
                    ...
                    for period, f_alt_scaling_nightglow_period in f_alt_scaling_nightglow.items():
                        ...
                        max_dist_loc = cns.get_max_dist(lats_stations, lons_stations, LATS, LONS, id_scenario, id_stat, period=period, f_alt_scaling=f_alt_scaling, **opt_dist)
                ...
                gdf_all['alt_ref_balloon'] = alt_ref_balloon
                ...
                file = f"./data/airglow_shp/{base_name}_SNRnight{dict_scaling['boost_SNR']['nightglow']}_SNRday{dict_scaling['boost_SNR']['dayglow']}_{type_detection}_m{model_subsurface}_20.12.2025.{ext}"
