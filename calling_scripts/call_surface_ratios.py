...
if __name__ == '__main__':
    ...
    """
    use_airglow_max_dist = True ## Use airglow
    airglow_and_balloon = False ## Include balloons with airglow
    offsets = [[20., 50.]]
    ...
    for SNR in SNRs:
        for type_airglow in ['dayglow', 'nightglow', ]:
            type_instruments = f'{type_airglow}_SNRnight{SNR:.1f}_SNRday{SNR:.1f}_same_event'
            
            #for model in ['Cold100', 'Hot10', 'Hot25', 'Hot40', ]:
            for model in ['Cold100',]:
                ...
                """
                folder_shp = 'airglow_shp'
                network = f'nightglow_dayglow_SNRnight1.0_SNRday1_same_event_m{model}'
                ext = 'shp'
                """
                """
                folder_shp = 'airglow_shp'
                ...
                network = f'{type_instruments}_m{model}_20.12.2025'
                ...
                alt_ref_balloon = gdf.alt_ref_balloon.iloc[0]
                ...
                for region, unioned_linestring in l_unioned_linestring.items():
                    ...
                    file = f'{dir_outputs}surface_ratios_{network}_{region_str}_20.12.2025.csv'
                    ...
                    #ratio_df.loc[(ratio_df.ratio>0)&(ratio_df.radius<1000e3)]
                    ratio_df['region'] = region_str
                    ratio_df['alt_ref_balloon'] = alt_ref_balloon
                    ...
                    ratio_df.to_csv(file, index=False, header=True)
