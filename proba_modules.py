...
class proba_model:
    ...
    def compute_proba_map(self, scenario, dists, M0s, SNR_thresholds, noise_level, duration, all_lats, all_lons, homogeneous_ratios, m_min, r_venus, return_rate=False, rates_provided=None, verbose=False, disable_tqdm=False):
        ...
        for ilon, lon in tqdm(enumerate(self.all_lons), total=len(all_lons), disable=disable_tqdm):
            ...
            #print(f'Time spent 1 {time.time()-time_loc} s')
            #time_loc = time.time()
            ...
            #print(f'Time spent 3 {time.time()-time_loc} s')
            #time_loc = time.time()
            ...
            #print(f'Time spent 4 {time.time()-time_loc} s')
...
def TL_converted_func(interpolator, dists, mws):
    if isinstance(dists, float):
        m_return = interpolator.ev(dists, mws)[0]
    else:
...
class proba_model_CPUs(proba_model):
    ...
    def compute_scores_across_CPUs(self, scenario, dists, M0s, SNR_thresholds, noise_level, duration, all_lats, all_lons, homogeneous_ratios, m_min, r_venus, nb_CPU=12):
        ...
        if N == 1:
            ...
            all_stacker_locator = [partial_compute_proba_map((0, idx_start_all, self.all_lons[idx_start_all]))]
...
def compute_multiple_trajectories(snrs, lats, lons, probas, winds, mission_durations, max_number_months, inputs):
    ...
    for lat, lon in tqdm(zip(LATS, LONS), total=LATS.size, disable=not icpu==0):
        ...
        new_trajectories = compute_proba_one_trajectory(trajectory, snrs, lats, lons, probas, snrs_selected=[1.,2.,5.], norm_factor_time=3600., disable_bar=True) ## Venusquake
